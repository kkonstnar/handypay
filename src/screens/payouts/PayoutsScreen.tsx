...
export default function PayoutsScreen(): React.ReactElement {
  ...
  // Use real Stripe API calls for payouts data
  const {
    data: apiPayouts,
    ...
    if (user?.id) {
      ...
      return apiService.getPayouts(user.id);
    } else {
      console.log('⚠️ No authenticated user data available for payouts fetch');
      return Promise.resolve({ data: [], success: true });
    }
  });

  // Use real Stripe API calls for balance data
  const {
    data: balanceData,
    ...
    if (user?.id) {
      ...
      return apiService.getBalance(user.id);
    } else {
      console.log('⚠️ No authenticated user data available for balance fetch');
      return Promise.resolve({ data: { balance: 0, currency: 'JMD' }, success: true });
    }
  });
  ...
  const {
    ...
    if (user?.id) {
      console.log('🔮 Fetching next payout for user:', user.id);
      return apiService.getNextPayout(user.id);
    } else {
      console.log('⚠️ No authenticated user data available for next payout fetch');
    ...
    return apiPayouts.map((payout) => {
      ...
      if (payout.payoutDate) {
        ...
        if (!isNaN(payoutDate.getTime())) {
          dateString = payoutDate.toISOString().split('T')[0];
        }
      } else if (payout.createdAt) {
        const createdDate = new Date(payout.createdAt);
        if (!isNaN(createdDate.getTime())) {
          dateString = createdDate.toISOString().split('T')[0];
        }
      }
  ...
  const handleShowSafetyPin = () => {
    console.log('handleShowSafetyPin called in PayoutsScreen');
